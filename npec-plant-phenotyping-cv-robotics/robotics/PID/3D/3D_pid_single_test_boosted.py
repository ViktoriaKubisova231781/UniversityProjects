...
def run_pid_trial_3d(kp, ki, kd, target, dt=1.0, max_steps=1000, threshold=0.001, hold_steps=50):
    ...
    # Save results to CSV
    filename = "pid_trial_3d_results_boosted_5.csv"
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("3d_pid_position_plot_boosted_5.png")
    plt.show()
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("3d__pid_distance_plot_boosted_5.png")
    plt.show()
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("3d_pid_error_plot_boosted_5.png")
    plt.show()
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("3d_pid_velocity_plot_boosted_5.png")
    plt.show()
...
if __name__ == "__main__":
    # Tuned PID gains for better convergence and stability
    kp = [15, 15, 15]
    ki = [0, 0, 0]
    kd = [0, 0, 0]
    dt = 0.50

    # Target: Bottom-right-back corner of workspace (plant location)
    target = np.array([0.10, 0.10, 0.13])

    print(f"\nRunning PID trial to target (inoculation corner): {target}")
    run_pid_trial_3d(kp, ki, kd, target, dt=dt, max_steps=1500)
