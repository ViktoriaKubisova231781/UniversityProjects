...
# ─── Logging Setup ────────────────────────────────────────
LOG_PATH = os.path.join("/tmp", "viki_inference_log.txt")  # Safe for Azure
...
def init():
    global model
    model_path = os.path.join(os.getenv("AZUREML_MODEL_DIR"), "12_viktoria_231781_unet_model_256px.h5")
...
def threshold_mask(raw_mask, threshold=0.1):
...
def run(raw_data):
    try:
        ...
        binary_mask = threshold_mask(mask, threshold=0.1)
